# Monkey patches to override default LLM behaviors in GraphRAG for embedding and search logic using a local Ollama model.

def patch_openai_embeddings_llm():
    """
    Monkey-patches the OpenAIEmbeddingsLLM class to use embeddings generated by a locally served Ollama model.
    
    Replaces the internal _execute_llm method with a custom async version that retrieves embeddings using Ollama.
    """
...
def patch_query_embedding():
    """
    Monkey-patches the OpenAIEmbedding class to replace both synchronous and asynchronous embedding logic 
    with a retry-enabled version using Ollama's local embeddings API.

    Adds custom retry logic using the Tenacity library for robust error handling.
    """
...
def patch_global_search():
    """
    Monkey-patches the GlobalSearch class to override the method responsible for generating responses 
    from structured community report chunks using a local LLM.

    Adds retry-based parsing and logging for handling noisy or malformed model responses.
    """
