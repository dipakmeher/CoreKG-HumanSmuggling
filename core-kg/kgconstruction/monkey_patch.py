# Overrides default LLM behaviors in the GraphRAG framework to enable local embedding and search logic using an Ollama-hosted model.

def patch_openai_embeddings_llm():
    """
    Replaces the internal embedding method of the OpenAIEmbeddingsLLM class 
    with a custom asynchronous implementation that uses embeddings generated 
    by a locally served Ollama model.
    
    Enables integration of local embedding models without modifying the original library code.
    """
...
def patch_query_embedding():
    """
    Replaces the default synchronous and asynchronous embedding methods 
    in the OpenAIEmbedding class with custom implementations that use 
    a locally served Ollama model.

    Incorporates retry logic using the Tenacity library to improve robustness 
    against transient failures during embedding generation.
    """
...
def patch_global_search():
    """
    Replaces the response generation method in the GlobalSearch class to enable querying 
    structured community report chunks using a locally served LLM.

    Includes enhanced error handling, retry-based parsing, and logging to improve robustness 
    against malformed or noisy model outputs.
    """
